...
<body class="min-h-screen bg-slate-100 flex items-center justify-center font-sans p-4">
  <div class="w-full max-w-sm bg-white rounded-2xl border border-slate-200 p-8 text-center">
    ...
    <p class="text-sm text-slate-500">The transaction was declined by Flutterwave.</p>
    @else
    ...
    <p class="text-sm text-slate-500">
      {{ $tx->currency }} {{ number_format($tx->amount / 100, 2) }} received.
    </p>
    @endif
    <p class="font-mono text-xs text-slate-400 mt-4">{{ $tx->reference }}</p>
  </div>
</body>
